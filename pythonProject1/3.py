...
# 打开文件
with open('testa.json', 'r', encoding='utf-8') as file:
    # 逐行读取
    for line in file:
        # 解析JSON对象
        json_obj = json.loads(line.strip())
        # 提取键的值
        value1 = json_obj.get('new-ID')
        value = json_obj.get('doc')
        text = re.sub(r'[[0-9]*]', ' ', value)  # 去除类似[1]，[2]
        ...
        f2 = '{' + '"summarizations":' + ' [{"id": "' + value1 + '_1", ' + '"event-summarization": "' + "".join(
            final_resul) + '"}]}'
        print(f2)
